...
public class ScheduleProxy {
	...
	public List<Schedule> alldataDB(){
		Schedule schedule = null;
		List<Schedule> scheduleList = new ArrayList<>();
		for(int i = 0 ; i<placeList().size();i++) {
			schedule = new Schedule(placeList().get(i),spotList().get(i),pointList().get(i),imgList().get(i),latitudeList().get(i),infoList().get(i));
			scheduleList.add(schedule);
		 }
		return scheduleList; 
	}
	public void insetSchedules() {
		List<Schedule> aList = alldataDB();
		for(int i = 0; i < aList.size();i++) {
			scheduleMapper.insertSchedule(aList.get(i));
		}
	}
}